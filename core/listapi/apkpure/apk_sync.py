...
class ApkPure:
    ...
    def _request(self, meth, url, *args, **kwgs):
        with self.session(http2=True, headers=self.hd) as req:
            res = req.request(meth, url, *args, **kwgs)
            return res if self._valid(res) else False

    def get_dw_url(self, u):
        response = self._request("GET", u)
    ...
    def search_apk(self, query: str):
        res = self._request("GET", self.search.format(query), headers=self.hd)
        if not res:
    ...
    def get_detail(self, url: str):
        res = self._request("GET", url, headers=self.hd)
        if not res:
            return {"status": False, "msg": "Unknown error"}
        ...
        u = self.get_dw_url(
            self.host+box.find("div", class_="ny-down").find("a", class_="da")["href"])
        ...
        r["desc"] = box.find("div", class_="description").find(
    ...
    def get_trending(self, page=1):
        res = self._request("GET", self.trending.format(page))
        if not res:
            return {"status": False, "msg": "Unknown error"}
